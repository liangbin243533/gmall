...
public class AttrGroupServiceImpl extends ServiceImpl<AttrGroupDao, AttrGroupEntity> implements AttrGroupService {
    ...
    public GroupVO queryGroupWithAttrsByGid(Long gid) {
        ...
        List<Long> attrIds = relations.stream().map(relationEntity ->
    ...
    public List<ItemGroupVO> queryItemGroupVOByCidAndSpuId(Long cid, Long spuId) {
        ...
        return attrGroupEntities.stream().map(group -> {
            ...
            //query attributes and value
            List<AttrAttrgroupRelationEntity> relationEntities = this.relationDao.selectList(new QueryWrapper<AttrAttrgroupRelationEntity>()
                    .eq("attr_group_id", group.getAttrGroupId()));
            List<Long> attrIds = relationEntities.stream().map(AttrAttrgroupRelationEntity::getAttrId).collect(Collectors.toList());
            List<ProductAttrValueEntity> productAttrValueEntities = this.attrValueDao.selectList(new QueryWrapper<ProductAttrValueEntity>()
                    .eq("spu_id", spuId)
                    .in("attr_id", attrIds));
